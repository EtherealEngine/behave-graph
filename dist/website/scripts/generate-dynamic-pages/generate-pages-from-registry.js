...
import { Graph, writeDefaultNodeSpecsToJSON, } from '@behave-graph/core';
...
const generateValuePages = (values, baseDir) => {
    ...
    writeFileSync(join(valuesDir, '_category_.json'), `{
  "label": "Values",
  "position": 1,
  "link": {
...
const generateNodePages = (nodes, baseDir, nodeSpecJson, graph) => {
    ...
    writeFileSync(join(nodesDir, '_category_.json'), `{
  "label": "Nodes",
  "position": 2,
  "link": {
    "type": "generated-index"
...
`);
    nodes.forEach((desc) => {
        ...
        writeFileSync(join(dirName, '_category_.json'), `{
...
`);
        console.log('Writing file', filePath);
        writeFileSync(filePath, nodeTemplate(nodeFactory(graph, {
            customEvents: {},
            variables: {}
        }), specJSON));
    });
...
export default (registry, baseDir, functionalRegistry) => {
...
//# sourceMappingURL=generate-pages-from-registry.js.map