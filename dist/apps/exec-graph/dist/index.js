...
import { DefaultLogger, Engine, Logger, LogLevel, ManualLifecycleEventEmitter, parseSafeFloat, readGraphFromJSON, registerCoreProfile, validateGraph, validateRegistry } from '@behave-graph/core';
...
async function execGraph({ jsonPattern, programOptions }) {
    const registry = registerSceneProfile(registerCoreProfile({
        values: {},
        nodes: {},
        dependencies: {
    ...
    if (programOptions.logLevel === LogLevel.Verbose) {
        engine.onNodeExecutionStart.addListener((node) => Logger.verbose(`<< ${node.description.typeName} >> START`));
        engine.onNodeExecutionEnd.addListener((node) => Logger.verbose(`<< ${node.description.typeName} >> END`));
    }
    ...
    Logger.verbose(`profile results: ${engine.executionSteps} nodes executed in ${deltaTime / 1000} seconds, at a rate of ${Math.round((engine.executionSteps * 1000) / deltaTime)} steps/second`);
    engine.dispose();
}
