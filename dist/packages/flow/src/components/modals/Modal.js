...
export const Modal = ({ open = false, onClose, title, children, actions }) => {
    ...
    return (<>
      <div className="z-[19] fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" onClick={onClose}></div>
      <div className="z-20 relative top-20 mx-auto border w-96 shadow-lg bg-white text-sm rounded-md">
        ...
        <div className="flex gap-3 p-3 border-t">
          {actions.map((action, ix) => (<button key={ix} className={'text-white p-2 w-full cursor-pointer ' +
                (ix === actions.length - 1
                    ? actionColors.primary
                    : actionColors.secondary)} onClick={action.onClick}>
              {action.label}
            </button>))}
        </div>
      </div>
