import { AsyncNode, EasingFunctions, EasingModes, NodeDescription, Socket, toCamelCase } from 'ee-behave-graph-core';
export class EaseSceneProperty extends AsyncNode {
    static GetDescriptions(scene, lifecycleEventEmitter, ...valueTypeNames) {
        return valueTypeNames.map((valueTypeName) => new NodeDescription(`scene/ease/${valueTypeName}`, 'Action', `Ease Scene ${toCamelCase(valueTypeName)}`, (description, graph) => new EaseSceneProperty(description, graph, valueTypeName, scene, lifecycleEventEmitter)));
    }
