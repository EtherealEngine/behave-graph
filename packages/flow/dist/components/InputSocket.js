...
const InputSocket = ({ connected, specJSON, ...rest }) => {
    ...
    return (_jsxs("div", { className: "flex grow items-center justify-start h-7", children: [isFlowSocket && (_jsx(FontAwesomeIcon, { icon: faCaretRight, color: "#ffffff", size: "lg" })), showName && _jsx("div", { className: "capitalize mr-2", children: name }), !isFlowSocket && !connected && _jsx(InputFieldForValue, { ...rest }), _jsx(Handle, { id: name, type: "target", position: Position.Left, className: cx(borderColor, connected ? backgroundColor : 'bg-gray-800'), isValidConnection: (connection) => isValidConnection(connection, instance, specJSON) })] }));
};
